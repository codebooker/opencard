...
# postgresql-client-16 (PGDG) matches the postgres:16 server for backup/restore.
# rclone sends optional manual backups to S3-compatible storage; gosu drops
# privileges after fixing writable volume ownership.
RUN apt-get update && apt-get install -y --no-install-recommends openssl ca-certificates curl gnupg gosu \
    ...
    && apt-get update && apt-get install -y --no-install-recommends postgresql-client-16 rclone tar \
